/*
 *  imupi.c: i2c access to the Invensense MPU-9150 Gyro, Accelerometer, and Compass chip
 *  
 *  JG, 1/1/2014. ROS adaption - TAU-adl, Omer Gavish
 */
...
int             imupi_dev         = -1;
unsigned char   mpu_9150_address  = MPU_9150_I2C_ADDRESS_2;
...
int imupi_init( void )  {
  ...
  if ( i2c_smbus_write_byte_data( imupi_dev, MPU_9150_SMPRT_DIV, 0x00 ) == -1 ) //1 Hz sampling rate.
...
int imupi_read( double *a, double *g, double *m ) {
  ...
  a[0] = (double)( (int16_t)( block[0] << 8 | block[1] ) * IMUPI_A_GAIN * M_G);
  a[1] = (double)( (int16_t)( block[2] << 8 | block[3] ) * IMUPI_A_GAIN * M_G);
  a[2] = (double)( (int16_t)( block[4] << 8 | block[5] ) * IMUPI_A_GAIN * M_G);
  ...
  g[0] = (double)( (int16_t)( block[0] << 8 | block[1] ) * IMUPI_G_GAIN * M_PI/180) - g_off[0];
  g[1] = (double)( (int16_t)( block[2] << 8 | block[3] ) * IMUPI_G_GAIN * M_PI/180) - g_off[1];
  g[2] = (double)( (int16_t)( block[4] << 8 | block[5] ) * IMUPI_G_GAIN * M_PI/180) - g_off[2];
  /*
